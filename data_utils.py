...
@dataclass
class Segment: # A time span in seconds, optionally carrying text
    start_s: float
    end_s: float
    ...
    def from_dict(cls, d: dict) -> "Segment":
        return cls(start_s=float(d["start"]), end_s=float(d["end"]), text=d.get("text"))


def parse_srt(path: Path) -> List[Segment]: # Parse an SRT subtitle file into a list of Segments (seconds)
    ...
    for sub in subs:
        ...
        if end > start: segments.append(Segment(start_s=start, end_s=end, text=sub.content))
    return sorted(segments, key=lambda s: s.start_s)


def parse_vtt(path: Path) -> List[Segment]: # Parse a WebVTT subtitle file into a list of Segments (seconds)
    segments = []
    for caption in webvtt.read(str(path)):
        start = _vtt_ts_to_seconds(caption.start)
        end = _vtt_ts_to_seconds(caption.end)
        text = caption.text.strip()
        if end > start and text: segments.append(Segment(start_s=start, end_s=end, text=text))
    return sorted(segments, key=lambda s: s.start_s)


def _vtt_ts_to_seconds(ts: str) -> float: # Convert a VTT timestamp string (HH:MM:SS.mmm) to seconds
...
def parse_subtitles(path: Path) -> List[Segment]: # Parse subtitles from either SRT or VTT format (auto-detected)
    suffix = path.suffix.lower()
    if suffix == ".srt": return parse_srt(path)
    elif suffix == ".vtt": return parse_vtt(path)
...
def parse_eaf_sentences(path: Path, tier: str = "SENTENCE") -> List[Segment]:
    ...
    # Falls back to tier names containing 'sentence' (case-insensitive) if the exact tier name is not found.
    eaf = pympi.Elan.Eaf(str(path))
    tier_names = eaf.get_tier_names()

    # Resolve tier name
    target = None
    if tier in tier_names: target = tier
    else:
        for t in tier_names:
            if "sentence" in t.lower():
                target = t
                break
    if target is None: raise ValueError(f"No SENTENCE tier found in {path}. Available tiers: {tier_names}")

    segments = []
    for start_ms, end_ms, value in eaf.get_annotation_data_for_tier(target):
        segments.append(Segment(
            start_s=start_ms / 1000.0, end_s=end_ms / 1000.0,
...
def get_video_duration(path: Path) -> float: # Return video duration in seconds using ffprobe (fast) or OpenCV (fallback)
    try: # Try ffprobe first (no decoding, instant)
    ...
    except (FileNotFoundError, ValueError, subprocess.TimeoutExpired): pass
    
    cap = cv2.VideoCapture(str(path)) # Fallback: OpenCV
    try:
        fps = cap.get(cv2.CAP_PROP_FPS)
        frame_count = cap.get(cv2.CAP_PROP_FRAME_COUNT)
        if fps > 0 and frame_count > 0: return frame_count / fps
    finally: cap.release()
    raise RuntimeError(f"Could not determine duration of {path}")


def get_video_fps(path: Path) -> float: # Return video FPS using OpenCV
...
def segments_to_binary(segments: List[Segment], duration_s: float, resolution_s: float = 0.04) -> np.ndarray:
...
_VTT_PATTERNS = [
    "{stem}.vtt", "{stem}.en.vtt", "{stem}.en-orig.vtt",
    "{stem}.en-en.vtt", "{stem}.en-GB.vtt", "{stem}.en-en-GB.vtt",
]
...
def discover_videos(data_dir: Path) -> List[dict]:
    ...
    for mp4 in mp4_files:
        ...
        if subs is None: continue  # skip videos without GT subtitles
        groups.append({"name": stem, "video": mp4, "subs": subs, "eaf": eaf if eaf.exists() else None})
    return groups


def save_segments_json(segments: List[Segment], path: Path) -> None: # Persist segments as a JSON file
    path.parent.mkdir(parents=True, exist_ok=True)
    with open(path, "w") as f:
        json.dump([s.to_dict() for s in segments], f, indent=2)


def load_segments_json(path: Path) -> List[Segment]: # Load segments from a cached JSON file
    with open(path) as f:
