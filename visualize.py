...
def plot_timeline(
    video_name: str,
    duration_s: float,
    method_segments: Dict[str, List[Segment]],
    output_dir: Path,
) -> Path:
    """Horizontal bar chart showing segments for each method on a shared time axis.

    *method_segments* maps method name (e.g. "GT", "Base", "Gemini", "Qwen")
    to its list of Segments.
    """
    ...
    for i, method in enumerate(methods):
        ...
        for seg in segments:
            ax.barh(y, seg.end_s - seg.start_s, left=seg.start_s,
                    height=bar_height, color=color, alpha=0.85, edgecolor="white",
                    linewidth=0.5)
    ...
    # Legend
    handles = [mpatches.Patch(color=METHOD_COLORS.get(m, "#95a5a6"), label=m)
               for m in methods]
...
def plot_metrics_summary(
    ...
    """Grouped bar chart of key metrics across methods.

    *all_metrics*: {method_name: {metric_name: [per-video values]}}
    """
    display_metrics = [
...
def plot_iou_distribution(
    ...
    for i, (d, color) in enumerate(zip(data, colors)):
        ...
        ax.scatter(np.full(len(d), i + 1) + jitter, d,
                   color=color, edgecolors="white", s=50, zorder=3, alpha=0.9)
...
def plot_segmentation_tendency(
    all_metrics: Dict[str, Dict[str, List[float]]],
    output_dir: Path,
) -> Path:
...
def plot_boundary_errors(
    boundary_errors_by_method: Dict[str, List[float]],
    output_dir: Path,
) -> Path:
...
def generate_all_plots(
    ...
    *video_results*: list of dicts, each with keys:
    ...
    for vr in video_results:
        pred_methods.update(k for k in vr["segments"] if k != "GT")
    ...
    for vr in video_results:
        for method in pred_methods:
            if method not in vr["metrics"]:
                continue
            for key, val in vr["metrics"][method].items():
                agg_metrics[method].setdefault(key, []).append(val)

            if method in vr.get("boundary_errors", {}):
                agg_boundary_errors[method].extend(vr["boundary_errors"][method])
...
def _is_active(t: float, segments: List[Segment]) -> bool:
    """Check if time *t* falls inside any segment."""
    for s in segments:
...
def _get_active_text(t: float, segments: List[Segment]) -> str:
    """Return the text of the GT segment that is active at time *t*."""
    for s in segments:
        if s.start_s <= t <= s.end_s:
            return s.text or ""
        if s.start_s > t:
...
def _draw_text(frame, text: str, pos: tuple, font_scale: float = 0.6,
               color=(255, 255, 255), thickness: int = 1, bg_color=None):
    ...
    if bg_color is not None:
        cv2.rectangle(frame, (x - 2, y - th - 4), (x + tw + 2, y + baseline + 2),
                      bg_color, cv2.FILLED)
...
def render_overlay_video(
    ...
    """Render an overlay comparison video.

    For each frame of the source video, draws:
    1. The original frame (top area)
    2. A panel showing method status bars (active = colored, inactive = dark)
    3. The GT subtitle text
    4. A full-duration mini-timeline at the bottom with playhead cursor
    ...
    bar_h = 28                           # height per method bar
    bar_gap = 4                          # gap between bars
    panel_pad = 12                       # padding inside panel
    subtitle_h = 50                      # height for subtitle text area
    timeline_h = 30                      # height for the mini-timeline
    panel_h = (panel_pad * 2
               + n_methods * bar_h
               + (n_methods - 1) * bar_gap
               + subtitle_h
    ...
    while True:
        ...
        # Create output frame: original on top, dark panel on bottom
        out_frame = np.zeros((out_h, out_w, 3), dtype=np.uint8)
        out_frame[:src_h, :, :] = frame

        # Semi-transparent dark panel background
        panel_top = src_h
        out_frame[panel_top:, :, :] = 30  # dark grey

        # ── Draw method status bars ────────────────────────────────────────
        bar_x = panel_pad + 80  # leave room for labels
        bar_w = out_w - bar_x - panel_pad
        y_cursor = panel_top + panel_pad

        for method in methods:
            segs = method_segments[method]
            active = _is_active(t, segs)

            color_hex = METHOD_COLORS.get(method, "#95a5a6")
            color_bgr = _hex_to_bgr(color_hex)
            dark_bgr = tuple(c // 4 for c in color_bgr)

            # Method label
            _draw_text(out_frame, method, (panel_pad, y_cursor + bar_h - 8),
                       font_scale=0.5, color=color_bgr, thickness=1)

            # Bar: filled if active, dark outline if not
            bar_rect = (bar_x, y_cursor, bar_x + bar_w, y_cursor + bar_h)
            if active:
                cv2.rectangle(out_frame, (bar_rect[0], bar_rect[1]),
                              (bar_rect[2], bar_rect[3]), color_bgr, cv2.FILLED)
                # Draw "ACTIVE" text inside
                _draw_text(out_frame, "SIGNING",
                           (bar_x + bar_w // 2 - 30, y_cursor + bar_h - 8),
                           font_scale=0.45, color=(255, 255, 255), thickness=1)
            else:
                cv2.rectangle(out_frame, (bar_rect[0], bar_rect[1]),
                              (bar_rect[2], bar_rect[3]), dark_bgr, cv2.FILLED)
                cv2.rectangle(out_frame, (bar_rect[0], bar_rect[1]),
                              (bar_rect[2], bar_rect[3]),
                              tuple(c // 2 for c in color_bgr), 1)

            y_cursor += bar_h + bar_gap

        # ── Draw GT subtitle text ─────────────────────────────────────────
        y_cursor += bar_gap
        gt_segs = method_segments.get("GT", [])
        subtitle_text = _get_active_text(t, gt_segs)
        if subtitle_text:
            # Word-wrap if text is too long
            max_chars = max(40, out_w // 12)
            lines = []
            for i in range(0, len(subtitle_text), max_chars):
                lines.append(subtitle_text[i:i + max_chars])
            for i, line in enumerate(lines[:2]):  # max 2 lines
                _draw_text(out_frame, line,
                           (panel_pad, y_cursor + 18 + i * 22),
                           font_scale=0.55, color=(255, 255, 255),
                           thickness=1, bg_color=(60, 60, 60))
        else:
            _draw_text(out_frame, "(no subtitle)",
                       (panel_pad, y_cursor + 18),
                       font_scale=0.45, color=(120, 120, 120), thickness=1)

        y_cursor += subtitle_h

        # ── Draw mini-timeline ────────────────────────────────────────────
        tl_x = panel_pad
        tl_w = out_w - 2 * panel_pad
        tl_y = y_cursor + bar_gap
        tl_h_per_method = max(4, timeline_h // n_methods)

        for i, method in enumerate(methods):
            segs = method_segments[method]
            color_hex = METHOD_COLORS.get(method, "#95a5a6")
            color_bgr = _hex_to_bgr(color_hex)
            row_y = tl_y + i * tl_h_per_method

            # Draw segment blocks on the timeline
            for seg in segs:
                if duration_s <= 0:
                    continue
                x1 = tl_x + int(seg.start_s / duration_s * tl_w)
                x2 = tl_x + int(seg.end_s / duration_s * tl_w)
                cv2.rectangle(out_frame, (x1, row_y), (x2, row_y + tl_h_per_method - 1),
                              color_bgr, cv2.FILLED)

        # Playhead cursor
        if duration_s > 0:
            cursor_x = tl_x + int(t / duration_s * tl_w)
            cv2.line(out_frame, (cursor_x, tl_y),
                     (cursor_x, tl_y + timeline_h), (255, 255, 255), 2)

        # Time display
        mins = int(t) // 60
        secs = t - mins * 60
        time_str = f"{mins}:{secs:05.2f} / {int(duration_s) // 60}:{duration_s % 60:05.2f}"
        _draw_text(out_frame, time_str,
                   (out_w - 200, panel_top + panel_h - 8),
                   font_scale=0.45, color=(200, 200, 200), thickness=1)
