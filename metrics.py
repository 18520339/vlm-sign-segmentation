...
def temporal_iou(pred: List[Segment], gt: List[Segment],
                 duration_s: float, resolution_s: float = 0.04) -> float:
...
# ── Segment F1 (pairwise IoU matching) ─────────────────────────────────────────
...
def _count_matches(
    query: List[Segment], pool: List[Segment], iou_threshold: float,
) -> int:
...
def segment_f1(pred: List[Segment], gt: List[Segment],
    ...
    IoU.  Count as a match only if IoU ≥ *iou_threshold*.

    This correctly penalises both:
...
def boundary_errors(pred: List[Segment], gt: List[Segment]) -> Dict:
    ...
    if not gt_boundaries or not pred:
        return {"errors": [], "mean_abs_error_s": float("nan"),
...
def count_ratio(pred: List[Segment], gt: List[Segment]) -> float:
    ...
    if not gt:
        return float("inf") if pred else 1.0
...
def compute_all_metrics(
    ...
    gt: List[Segment],
    duration_s: float,
    resolution_s: float = 0.04,
    iou_thresholds: List[float] = None,
) -> Dict[str, float]:
    """Run every metric and return a flat dictionary.

    Segment F1 is computed at each IoU threshold in *iou_thresholds*
    (default: [0.3, 0.5, 0.7]).  This follows the ActivityNet convention
    for temporal action detection evaluation.
    """
    ...
    # Coverage metric
    result["temporal_iou"] = temporal_iou(pred, gt, duration_s, resolution_s)

    # Segment F1 at multiple IoU thresholds
